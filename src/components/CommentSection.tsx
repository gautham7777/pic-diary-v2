...
const CommentSection: React.FC<{ photo: Photo }> = ({ photo }) => {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    // FIX: tempComment is declared outside the try block to be accessible in the catch block for UI reversal on failure.
    const tempComment = { text: newComment, username: "You", createdAt: new Date(), isUserComment: true };
    try {
      ...
      setComments(prev => prev.filter(c => c.createdAt !== tempComment.createdAt));
    } finally {
  ...
  const handleGenerateComment = useCallback(async () => {
    ...
    if (!process.env.API_KEY) { setError("Gemini API key is not configured."); return; }
    ...
    try {
        ...
        const textPart = { text: "You are a friendly social media user. Write a short, positive, complimentary comment for this photo, under 15 words, with an emoji. Example: 'So beautiful! 😍'" };
    ...
    <div className="w-full max-w-lg mx-auto mt-4 pt-4 border-t border-accent-soft/50">
        ...
        <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
            ...
            {comments.map((comment, index) => (
                <div key={index} className={`p-3 rounded-lg transition-colors ${comment.isUserComment ? 'bg-accent/10' : 'bg-secondary-bg/50'}`}>
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-secondary-text text-sm whitespace-pre-wrap">{comment.text}</p>
                        </div>
                    </div>
                </div>
            ))}
